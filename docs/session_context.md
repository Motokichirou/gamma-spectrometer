...
> Обновлять перед каждым сжатием. Дата последнего обновления: 2026-06-14 (сессия 10).

---

## ⚡ БЫСТРЫЙ СТАРТ ПОСЛЕ СЖАТИЯ (обновлено 2026-06-14, сессия 10)

### ПК-софт `host/gammapult` — текущий главный фронт (v1 связь+диагностика+калибровка ГОТОВ)
Нативное Windows-приложение: C++ / Win32 / DirectX 11 / Dear ImGui + ImPlot, **единый .exe без зависимостей** (статический CRT). Сборка MSVC (VS Build Tools 2022).
**Двойной режим из одного кода (`host/src/entry.cpp`):** `gammapult.exe` (WINDOWS subsystem) = GUI; `gammapult.com` (CONSOLE) = CLI (cmd сам берёт .com). CLI-подкоманды: `list/info/acquire/enc/cmd`.
```bash
CMAKE="C:/Program Files (x86)/Microsoft Visual Studio/2022/BuildTools/Common7/IDE/CommonExtensions/Microsoft/CMake/CMake/bin/cmake.exe"
"$CMAKE" -G "Visual Studio 17 2022" -A x64 -S host -B host/build   # configure (1 раз; FetchContent тянет imgui v1.90.9 + implot v0.16)
"$CMAKE" --build host/build --config Release                        # сборка
# ПЕРЕД сборкой: taskkill //F //IM gammapult.exe (иначе LNK1104 — exe запущен)
```
GUI v1: подключение, live-спектр (Лин/Лог; колесо=X, Shift+колесо=Y; оси не уезжают за [0..8192]/<0; Y авто-подгон), телеметрия, консоль с чипами команд, самотест ENC (таблица+графики линейности/FWHM), Справка, **раздвижные панели**, настраиваемый порог DSP, **калибровка энергий** (канал→кэВ, полином 1..4, проверка валидности `CheckCalibration` по образцу BecqMoni: монотонность+границы, запись во flash).
Скриншот GUI для проверки: Start-Process → ShowWindow(SW_MAXIMIZE)+SetForegroundWindow → GetWindowRect → CopyFromScreen → PNG → Read. ⚠ Если окно не вышло на передний план — снимется ЧУЖОЕ окно (приватность!). Логику лучше проверять детерминированно в Python.

### Новые команды прошивки (сессия 10)
- `-tst ra` — радиевый тест-спектр (`invcdf_ra.h` из `радий.xml`; `-tst spec` = Cs-черника). `pulsegen_set_spectrum(0|1)`.
- `-thr <каналы>` — порог DSP в каналах (1 код = 2 канала, т.е. codes=каналы/2); `-thr` без аргумента = запрос. **Дефолт 50 каналов** (=25 кодов, оптимум на петле Nucleo).
- `-wcal <order> <c0..cN>` — запись энергокалибровки нашим тулом во flash G474 (`cal_store.c`, 0x0807F800 = Bank2 page127 dual-bank, magic `GCA2`+CRC16, **переживает ребут**). Хранит **11 сырых слов** формата BecqMoni; сумму (слово 10) считает сам.
- `-rcal` — НАШ читаемый формат: `CAL: <order> c0 c1 ..\r\n<серийник>\r\n` (для gammapult; не для BecqMoni). Голый `-cal` теперь отдаёт формат BecqMoni — поэтому host connect шлёт `-rcal`.
- `-calclr` — стереть сохранённую калибровку (страница → 0xFF), ответ `-ok cal cleared`.
- `-dbg` / `-dbgc` — ⚠**ВРЕМЕННО** (реверс BecqMoni): захват входящих команд в RAM-буфер `dbg_buf` (app.c). **НЕ закоммичено**, убрать.

### ✅ РЕШЕНО: кнопки калибровки BecqMoni работают с нашим прибором (сессия 10)
BecqMoni у пользователя **новее** публичного `Am6er/BecqMoni` master (там нет формы калибровки с «Читать/Записать с устройства»). Протокол разгадан полностью реверсом по `-dbg` + диалогу ошибки.

**ЗАПИСЬ «Записать в устройство»** = 11 команд `-cal <i> <hex32>`, i=0..10, каждая ждёт ACK `-ok`:
```
слова 0-9 = 5 коэффициентов c0..c4 (IEEE-754 double), пара (чётный=ст.слово, нечётный=мл.слово), big-endian:
  (0,1)=c0  (2,3)=c1  (4,5)=c2  (6,7)=c3  (8,9)=c4 ; u64=(w[2k]<<32)|w[2k+1] → double
слово 10 = КОНТРОЛЬНАЯ СУММА (разгадана): стандартный CRC-32 (poly 0x04C11DB7 reflected=0xEDB88320,
  init/xorout 0xFFFFFFFF — обычный zip/zlib.crc32) по ASCII-строке из слов 0..9 как "%08X"
  (ВЕРХНИЙ регистр, 8 цифр, без разделителей, 80 символов). Проверено на 2 реальных записях BecqMoni.
```
**ЧТЕНИЕ «Читать с устройства»** = голый `-cal` (та же команда, что и проверка связи). Прибор отдаёт 11 слов (по строке, `%08X`) + серийник + пустая строка; BecqMoni парсит 11 слов и проверяет CRC-32 слова 10. Проверка связи берёт предпоследнюю строку как серийник — формат это сохраняет.

**Реализация (готова, прошита, проверена сквозняком):**
- `cal_store.c`: хранит 11 сырых слов; `app_cal_stage_word(i,word)` копит запись BecqMoni (шьёт по приходу всех 11); `app_cal_read_words` отдаёт; `app_cal_write(order,coef)` (наш `-wcal`) строит слова из doubles и сам считает `becq_checksum()` (CRC-32 по ASCII); `app_cal_clear`.
- `app.c handle_text`: `-calclr` (до `-cal`!), `-rcal`, `-cal` (голый → 11слов+SN или `CAL: none`; `-cal i hex` → stage+`-ok`; `-cal i` → слово).
- host `device.cpp` connect шлёт `-rcal`; `main.cpp` подтверждение после `-wcal` тоже `-rcal`.
- Проверка: `-wcal 1 10 0.5` → слово10=`4FF3D93E` = zlib.crc32 эталон ✓. Запись из BecqMoni хранится дословно (сумма верная) ✓.
- ⚠ Запись из BecqMoni роняла COM, пока в flash лежала калибровка с фейковой суммой (BecqMoni пред-читает `-cal`, видит плохой CRC, бросает исключение, закрывает порт). С валидной/пустой калибровкой запись проходит.

### Git (сессия 10)
HEAD = `c88f47d`, **запушено** в origin. Незакоммичено: `cal_store.c` (11 слов+CRC32+clear), правки `-cal/-rcal/-calclr` в `app.c`/`app.h` (обе копии), host `device.cpp`/`main.cpp` (`-rcal`), + временный `-dbg` в `app.c`. Прибор прошит этой сборкой. **Финализация:** убрать `-dbg`, собрать+прошить, закоммитить+запушить.

---

### (ниже — состояние железа/прошивки на 2026-06-13, сессия 9)
...
**Генератор при старте ВЫКЛ** (cur_mode=PG_OFF); включается -tst spec|mono|enc, глушится -tst off.
На боевой плате вход АЦП — реальный сигнал ФЭУ, генератор сам стартовать не должен.
