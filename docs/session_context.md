...
- «Обновить плату из схемы» (F8) — затянуты все footprint'ы; ⚠ ошибка `J_PMT401 (нет footprint)` — ожидаемо (кастомный B14-38 делителя, отложен). Группа 1xx переселена в круг (выбор «элементы того же листа» + M).
...
1. **Разводка PCB** — все 4 схемы готовы (ERC 0, BOM с MPN полный), разводка НЕ начата. Главный следующий блок. Предложение: начать с USB/Power либо Divider. ⚠ перед заказом проверить footprint D202/D203 (`D_SOD-123`) и кастомный J_PMT401 (B14-38).
