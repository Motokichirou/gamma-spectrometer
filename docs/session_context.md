...
- `-wcal <order> <c0..cN>` — запись энергокалибровки во внутр. flash G474 (`cal_store.c`, адрес 0x0807F800 = Bank2 page127 dual-bank, magic+CRC16, **переживает ребут**). `-cal` отдаёт сохранённые коэф.: `CAL: <order> <c0..>\r\n<серийник>\r\n` (серийник предпоследней строкой — хэндшейк BecqMoni цел).
- `-dbg` / `-dbgc` — ⚠**ВРЕМЕННО** (реверс BecqMoni): захват входящих команд в RAM-буфер `dbg_buf` (app.c). **НЕ закоммичено**, убрать после финализации.

### 🔴 АКТИВНАЯ ЗАДАЧА: заставить кнопки калибровки BecqMoni работать с нашим прибором
BecqMoni у пользователя **новее** публичного `Am6er/BecqMoni` master (в master НЕТ формы энергокалибровки с «Читать/Записать с устройства»). Реверсим протокол по диалогу ошибки BecqMoni + захвату `-dbg`.
**ЗАПИСЬ «Записать в устройство» — РАСШИФРОВАНА** (11 команд):
```
-cal 0 <hi32>  -cal 1 <lo32>  = c0(e) как IEEE-754 double, СТАРШЕЕ слово первым (big-endian)
-cal 2..3 = c1(d) ;  -cal 4..5 = c2(c) ;  -cal 6..7 = c3(b) ;  -cal 8..9 = c4(a)
-cal 10 <hex>  = контрольная сумма (алгоритм НЕ опознан: не CRC32/CRC16-modbus/sum/xor — пробовать ещё, либо просто хранить/отдавать как есть)
```
Каждая `-cal i hex` ждёт ACK → BecqMoni показывает `result: True`. Сейчас наш `-cal` на любой `-cal …` отдаёт читалку `CAL: …` → это НЕ ack → `False`. **Ожидаемый ack неизвестен, лучшая гипотеза `-ok`.** Декод подтверждён: пары (idx even=hi, odd=lo, big-endian) как double = ровно коэффициенты пользователя.
**ЧТЕНИЕ «Читать с устройства» — НЕ пойман.** Буфер `-dbg` очищен, ждём: пользователь жмёт «Читать с устройства» → закрывает BecqMoni → читаем `gammapult.com cmd --port COM6 --send "-dbg"`. Гипотезы: `-cal <i>` поканально ЛИБО голый `-cal`.
**ПЛАН (firmware `app.c` handle_text):** различать `-cal` (без арг → серийник как сейчас) / `-cal <i> <hex>` (store word[i] i=0..10, reply `-ok`) / `-cal <i>` (вернуть hex слова i). Хранить 11 сырых uint32 во flash (сумму не считать — word10 хранить/отдавать как есть → write и read-back совпадут). Реконструкция 5 doubles из слов 0-9 (пары hi,lo BE) — для нашего тула. shproto НЕ трогаем (команды уровня приложения).

### Git (сессия 10)
HEAD = `c88f47d`, **запушено** в origin. Незакоммичено: временный `-dbg`-захват в `firmware/core/app.c` (+ копия в `gamma_stage1`). Прибор ПРОШИТ этой временной сборкой. Убрать/закоммитить при финализации протокола калибровки.
