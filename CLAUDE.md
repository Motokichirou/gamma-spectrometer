...
6. **v2 (будущий апгрейд):** MCU board v2.0 на AD4084 (16-bit 20 MSPS DPP) — полное ТЗ в `docs/ad4084_dpp_design.md`. Решение 2026-06-10: сначала v1 в железо. v2 = замена только MCU board в стеке.
   - **Вход делителя v2:** AD8000 → **ADA4817-1 как TIA** (drop-in по корпусу/распиновке; −18% шума, идеальная линейность; подтверждено `simulation/sim_tia_real.cir`). Детали и remove/add — в `divider_board.md`. На разрешение v1 не влияет (кристалл-лимит) → только v2/запас.
