...
1. **Полный BOM с MPN/footprint** — ✅ ВЫПОЛНЕНО (сессия 5, 2026-05-23): см. `bom_fp_work.md`. Актуальный счёт по `pcb/gamma_spectrometer.net` (185 компонентов): 81 конденсатор / 17 MPN, **67 резисторов** (прежние «79» — до переноса AD8000 на делитель в v1.6 и фикса C1), индуктивности/LED/феррит/предохранитель.
2. **Разводка PCB** — 🚧 В ПРОЦЕССЕ. Все 4 платы в одном `gamma_spectrometer.kicad_pcb` (KiCad 9, 4 слоя: F.Cu / In1.Cu-GND / In2.Cu-GND / B.Cu). **USB/Power**: разведена, односторонний монтаж (всё на F.Cu), GND сшит вианами, DRC по электрике чист (паритет со схемой 0). **HV**: разведена, GND-полигоны залиты. **Межплатная шина**: цанг-пины `INTERCONNECT_PIN_D1.5` — по 13 на USB и HV, выровнены на дуге R20, цепи назначены, соосны через стек; латунные стойки `STANDOFF_M2.5_GND_x3_TRIANGLE` в центре каждой платы (GND). ⚠ Кросс-платные «не подключено» в DRC — норма (платы лежат рядом в одном файле, мост = цанг-пины в стеке). **Впереди**: MCU и делитель + копия шины на MCU. Детали — `docs/session_context.md`.
...
5. ⚠ **К проверке перед заказом:** D202/D203 footprint должен быть `Diode_SMD:D_SOD-123` (BZT52C18 Diotec в стандартном SOD-123, не -123W). Футпринты межплатки/ФЭУ сделаны (сессия 14, 2026-06-16) в библиотеке `посадочные места`: `INTERCONNECT_PIN_D1.5`, `STANDOFF_M2.5_GND`(+`_x3_TRIANGLE` R19), `J_PMT401_R1307-01` (реальный обмер: 18 слотов, круг уменьшен Ø35→Ø30 под стойки). Проверить линейкой в редакторе перед заказом.
6. **v2 (будущий апгрейд):** MCU board v2.0 на AD4084 (16-bit 20 MSPS DPP) — полное ТЗ в `docs/ad4084_dpp_design.md`. Решение 2026-06-10: сначала v1 в железо. v2 = замена только MCU board в стеке.
7. **✅ Делитель пересобран целиком (2026-06-24)** — активный делитель приведён к Hamamatsu Fig 5-15 + вход на TIA. Подтверждено единой моделью `simulation/sim_divider_complete.cir`:
