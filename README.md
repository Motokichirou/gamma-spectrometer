...
- **Сигнал**: коаксиал RG-178 + U.FL (делитель → МК, ~470 мВ для Cs-137)
...
─────────────    ─────────────────────────────   ──────────────────────────────────
 ...
 I_анода(Cs-137)  ADA4817 TIA, Zt=499 Ω, BW≈92 МГц →  PGA ×4 (OPAMP3)
  = 0.94 мА       V_вых = +469 мВ (Cs-137)         V_ADC = baseline 0.408 В
                                                   ADC1_IN12, 4 Мвыб/с
                                                   гистограмма 8192 кан. → USB
```

**Полярность АЦП:** анод забирает ток ⇒ выход TIA вверх ⇒ импульсы **положительны** относительно baseline 0.408 В → `amplitude = sample − baseline`
...
| LTspice: вход TIA (ADA4817) + делитель | ✅ Валидировано (`sim_divider_complete`: Zt=499 Ω, буферы смещены ~96 мкА, диоды B-E) |
| LTspice sim2: шейпер CR-RC² | ✅ Готово (PF=0.202, ширина импульса ~1.5 мкс) |
| LTspice sim3: бюджет шумов | ✅ Готово (σ=76.9 мкВ rms с TIA, 0.25 кан.; 94.4 с прежним AD8000) |
| BOM с MPN + footprint | ✅ Готово (177 комп.: 74 конд. / 17 MPN, 63 R, все активные) |
| Разводка PCB | 🚧 2 слоя, все 4 платы разведены, паритет со схемой 0. Нормы ВВ-зазоров (3 мм/кВ) заданы в `.kicad_dru`. ⚠ Осталось развести 59 мест, где земля подходит к ВВ-цепям ближе нормы |
...
└── simulation/
    ├── models/                       # SPICE-модели (opamp_g474, ADA4817, AD8000)
